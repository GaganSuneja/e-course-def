...
type Props = {
    submit:(form) => void, isShowing:boolean, hide:()=>void, 
    edit:(form)=>void,
    userDetails: {
        _id:string,
        role:string,school:string,gender:string,
        name:string,email:string,contact:string,designation:string} 
}

const InputFormModal = (props:Props) => {
    const [userForm,setUserForm] = useState(props.userDetails?props.userDetails:{ role:"user",school:"Darshan Academy, HQ",gender:"F",
    ...
    useEffect(()=>{
        if(!props.userDetails) {
            setUserForm({ role:"user",school:"Darshan Academy, HQ",gender:"F",
            name:"",email:"",contact:"",designation:""});
        } else {
    ...
    const handleSubmit = (event)=> {
        ...
        if(props.userDetails && props.userDetails._id) {
            props.edit(userForm);
        }else {
            props.submit(userForm);
        }   
    }
                    ...
                    <div className="custom-modal">
                        ...
                        <form onSubmit={handleSubmit} >
                            ...
                            <div className="form-group">
                                <label>Gender</label>
                                <br/>
                                <select name="gender" style={{width: '50%',border: '1px solid #ced4da',height: '32px',borderRadius: '5px'}} 
                                 onChange={handleFormChanges} value={userForm.gender}>
                            ...
                            <div className="form-group">
                                <label>School</label>
                                <br/>
                                <select name="school" style={{width: '50%',border: '1px solid #ced4da',height: '32px',borderRadius: '5px'}}
                                 onChange={handleFormChanges}> value={userForm.school}
                            ...
                            <div className="form-group">
                                <label>Role</label>
                                <br/>
                                <select name="role" value={userForm.role} style={{width: '50%',border: '1px solid #ced4da',height: '32px',borderRadius: '5px'}}  onChange={handleFormChanges}>
                            ...
                            <button className="btn btn-primary btn-block">
                                {props.userDetails ? 'Edit Account': 'Create Account'}
                            </button>
                        </form>
                    </div>
                </div>
