...
import CustomInputField, { HandleInputResponse } from './customInputField/customInputField';
  ...
  return (
    <div className="row" style={{ marginBottom: '2%' }}>
      <div className="col-1" style={{ textAlign: 'center' }}>
        <CustomInputField id={id} handleFieldClick={handleResponse} submittedValue={submittedAnswer}/>
      </div>
...
const Rating = (props: Props) => {
  ...
  let submittedRating =  useRef<Array<{ question: string, _id: string, rating: string }>>(JSON.parse(JSON.stringify(questions)));
  ...
  return (
    <>
      ...
      <br/>
      {!props.submittedResults && props.showMessageForUnfilled && <RedoOverlay text="Please fill rating on previous page"/>}
      <div className="container" style={{ filter: (!props.submittedResults && props.showMessageForUnfilled) ? 'blur(5px)' : '' }}>
      ...
      <div style={{ textAlign: 'center' }}>
        { (props.showSubmit === undefined || props.showSubmit === true)  && <PrimaryButton OnClick={e => saveResults()}
                       btn_text={'Submit'}
