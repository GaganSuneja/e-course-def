...
type HandleInputResponse = (index: string, response: string | undefined) => void;
...
const CustomInputField = ({
  ...
  const [inputValue, setInputValue] = useState<string | undefined>(submittedValue ? submittedValue : '');
  const [isFieldDisabled, setIsFieldDisabled] = useState<boolean | undefined>(false);
  ...
  const updateInput = (event): void => {
    if (event) {
      if (!event.target.value.match(/^[1-5]$/)) {
        setShowPopup(true);
        setInputString(<p style={{ textAlign: 'center', fontFamily: 'Rubik,san-serif' }}>Please Enter Value Between 1
          and 5</p>);
        setInputValue('');
        setFieldValue('');
      }  else {
  ...
  return (
    <div>
      ...
      />
      <RatingModal isShowing={showPopup} hide={closeForm} element={inputString}/>
    </div>
  );
};
