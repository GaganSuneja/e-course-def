...
import { MODULE_DATA, RATING, TRUE_FALSE, MCQ_QUESTION, USER, USER_PROFILE, _ID, override, FILL_IN_BLANK } from '../../../constants';
...
const fillInBlankDefault : fillInBlankDataProps = {
  options: [],
  questionGroup: []
} ;

const ActivityWrapper = (props: Props) => {
  const [loader, setLoader] = useState<boolean>(false);
  const [submittedResults, setSubmittedResults] = useState<boolean>(false);
  const [showResults, setShowResults] = useState<boolean>(false);
  const [data, setData] = useState<Array<any>>([]);
  const [fillInBlanksData, setFillInBlanksData] = useState<fillInBlankDataProps>(fillInBlankDefault);
  ...
  const isModuleIncomplete = useSelector(
    (state) =>
      state[MODULE_DATA]?.[`${props.activityInfo.moduleId}`]
  );
      ...
      (async () => {
        try {
          ...
          if (res.userSubmitted) {
            ...
            if(props.activityType === FILL_IN_BLANK ) {
              setFillInBlanksData(res.pickCount ? randomiseData(res.activities, res.pickCount) : res.activities)
            } else {
              setData(res.pickCount ? randomiseData(res.activities, res.pickCount) : res.activities);              
            }
  ...
  const saveResponse = useCallback((response) => {
    ...
    saveActivityResult(userId, props.activityInfo.moduleId, progressObject).then(() => {
      setShowResults(true);
      if(props.activityType === FILL_IN_BLANK ) {
  ...
  const reloadActivity = async () => {
    ...
    try {
      ...
      if(props.activityType === FILL_IN_BLANK ) {
        setFillInBlanksData(true ? randomiseData(res.activities, res.pickCount):[]);
      } else {
  ...
  if (showModal) {
    ...
    switch (props.activityType) {
      case TRUE_FALSE: {
        return (
          <TrueFalse deleteResponse={deleteUserActivity}
      ...
      case RATING: {
        return (
          <Rating submittedResults={submittedResults}
                  showResults={showResults}
                  data={data}
      ...
      case MCQ_QUESTION: {
        return (
          <Mcq submittedResults={submittedResults}
               showResults={showResults}
               data={data}
               saveResponse={saveResponse}
